...
- [x] [S] [ops][010] **Deploy GitHub Pages + migrar repo a github.com/Ledder-Dev** — usuario confirmó repo público. Repo nuevo `Ledder-Dev/finances-frontend` creado con historia completa (`master`+`development` pusheados), repo viejo `camachoeng/finances-frontend` intacto como remote secundario `camachoeng`. `vite.config.js` con `base: '/finances-frontend/'`, workflow `.github/workflows/deploy-pages.yml`, Pages configurado con `build_type: workflow`, default branch del repo nuevo puesto a `master`, política de deployment branch del environment `github-pages` ampliada pa aceptar `master` (venía solo con `development` por default branch inicial). Deploy verificado exitoso: `https://ledder-dev.github.io/finances-frontend/` live. Pendiente: `finances-backend` sin URL pública HTTPS todavía — login va a fallar hasta que el backend también esté desplegado públicamente (documentado en ADR 003, fuera de este mundo). (2026-08-04)
